    ...
    <tbody>
            ...
            @foreach($dataList as $item)
             <tr>
                ...
                <td>
                    @if($item->status == 0 )
                        <label class="label-warning">Pending</label>
                    @elseif($item->status == 1)
                        <label class="label-success">Approved</label>
                    @else
                        <label class="label-danger">Cancel</label>
                    @endif
                </td>
                <td>{{ number_format($item-> total_money,0) }}</td>
                <td>
                     @if($item->status == 0 )
                        <button class="btn btn-success" onclick="updateItem({{$item->id}} ,1)">Approve</button>
...
<script type="text/javascript">
    function updateItem(id,status) {
        if(status == 1){
            var option = confirm('Are you sure update Order status "Approved"?')
        }else {
            var option = confirm('Are you sure update Order Cancel ?')
        }
        if(!option) return$
      ...
      function deleteItem(id) {
        var option = confirm('Are you sure delete Order ?')
        if(!option) return$
